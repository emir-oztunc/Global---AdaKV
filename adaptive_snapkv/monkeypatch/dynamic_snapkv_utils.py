    ...
    def update(self, key_states, value_states, layer_idx, cache_kwargs=None):
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            try:
                ...
                # Python fallback
                heads = head_lens.shape[0]
                flat_k = self.key_cache[layer_idx].view(-1, dim)
                flat_v = self.value_cache[layer_idx].view(-1, dim)
                new_k = key_states.view(heads, dim)
                new_v = value_states.view(heads, dim)
                ...
                for i in range(heads):
                    start = cu_klen[i]
                    length = head_lens[i]
                    updated_k.append(flat_k[start : start + length])
                    updated_k.append(new_k[i].unsqueeze(0))
                    
                    updated_v.append(flat_v[start : start + length])
                    updated_v.append(new_v[i].unsqueeze(0))
                
                new_key_cache = torch.cat(updated_k, dim=0)
                new_value_cache = torch.cat(updated_v, dim=0)
...
class AdaptiveSnapKVCluster():
    ...
    def init_metadata(self, num_heads, k_lens, klen_sum, max_seqlen_k, device):
        # init metadata
        ...
        self.cu_klen = torch.cat(
            [self.cu_klen, torch.tensor([self.klen_sum], dtype=torch.int32, device=device)], dim=0)
        
        # check bug
        self.layer_qlens = torch.ones(num_heads//self.num_key_value_groups, dtype=torch.int32, device=device)
        ...
        self.cu_qlen = torch.cat(
            [self.cu_qlen, torch.tensor([self.qlen_sum], dtype=torch.int32, device=device)], dim=0)
        
        if self.gqa_support:
            self.cu_offset = torch.arange(0, num_heads//self.num_key_value_groups + 1, dtype=torch.int32, device=device)
            self.cu_head_offset = torch.arange(1, num_heads//self.num_key_value_groups + 1, dtype=torch.int32, device=device)
        else:
            self.cu_offset = torch.arange(0, num_heads + 1, dtype=torch.int32, device=device)
            self.cu_head_offset = torch.arange(1, num_heads + 1, dtype=torch.int32, device=device)
    ...
    def calculate_entropy(self, attn_score):
        # attn_score is already softmaxed (probabilities summing to 1)
        # Add a tiny epsilon to prevent log(0) which causes NaN errors
        epsilon = 1e-9
        
        # Calculate entropy along the sequence dimension
        entropy = -(attn_score * torch.log(attn_score + epsilon)).sum(dim=-1)
        
        # Average the entropy across all heads for this specific layer
...
def perform_cross_layer_pruning(model, past_key_values, dynamic_head_budgets, config):
    """
    Applies the dynamically calculated cross-layer budgets to prune the KV cache 
    right before the decoding phase begins.
    """
    print("Starting global cross-layer pruning...")
    
    # Extract config variables we need
    window_size = config.window_size
    floor_ratio = getattr(config, 'floor_alpha', 0.2)
    num_heads = config.num_attention_heads
    num_key_value_groups = num_heads // getattr(config, 'num_key_value_heads', num_heads)
    
    pruning_stats = {}
    
    for layer_idx in range(len(past_key_values.key_cache)):
        # 1. Retrieve the saved, uncompressed data
        key_states = past_key_values.key_cache[layer_idx]
        value_states = past_key_values.value_cache[layer_idx]
        attn_score = past_key_values.attn_scores[layer_idx]
        
        bsz = key_states.shape[0]
        q_len = key_states.shape[2] # original sequence length
        head_dim = key_states.shape[3]
        
        _device = key_states.device
        
        # 2. Sort the attention scores to find the most important tokens
        # Note: In dynamic_llama_hijack.py, attn_score was generated using key_states_rep
        # and has shape [bsz, num_heads, length]. But we only have num_key_value_heads physical blocks.
        # We need to mean/max pool the attention scores over the groups before sorting.
        if attn_score.shape[1] == num_heads and num_key_value_groups > 1:
            attn_score = attn_score.view(bsz, num_heads // num_key_value_groups, num_key_value_groups, -1).max(dim=2).values

        sorted_attn_score, sorted_attn_score_indices = attn_score.sort(dim=-1, descending=True)
        length = sorted_attn_score.size(dim=-1)
        
        layer_base_capacity = sum(dynamic_head_budgets[layer_idx]) // (num_heads // num_key_value_groups)
        
        # We have exact sizes for every head already
        head_adaptive_capacity = torch.tensor([dynamic_head_budgets[layer_idx]], device=_device, dtype=sorted_attn_score_indices.dtype)
        
        # Collect stats for logging
        epsilon = 1e-9
        _score = attn_score.float()
        head_entropies = -(_score * torch.log(_score + epsilon)).sum(dim=-1)
        
        pruning_stats[f"layer_{layer_idx}"] = {
            "layer_budget": sum(dynamic_head_budgets[layer_idx]),
            "layer_entropy": float(head_entropies.mean().item()),
            "head_budgets": head_adaptive_capacity[0].tolist(),
            "head_entropies": head_entropies[0].tolist()
        }
        ...
        # 4. Actually slice the tensors
        for head_idx in range(num_heads // num_key_value_groups):
            # Grab the indices for the tokens we want to keep
            cache_index = sorted_attn_score_indices[head_idx][..., :head_adaptive_capacity[0][head_idx]]
            cache_index = cache_index.view(1, 1, -1, 1).expand(-1, -1, -1, head_dim)
            
            # Gather the important past tokens
            top_Kcache = origin_heads_key_states[head_idx].gather(dim=2, index=cache_index)
            top_Vcache = origin_heads_value_states[head_idx].gather(dim=2, index=cache_index)
            
            # Concatenate them with the recent observation window
            selected_k = torch.cat([top_Kcache, origin_heads_key_states[head_idx][:, :, -window_size:, :]], dim=2)
            selected_v = torch.cat([top_Vcache, origin_heads_value_states[head_idx][:, :, -window_size:, :]], dim=2)

            # ---------------------------------------------------------
            # 2. Add the length tracking RIGHT HERE, before you flatten it
            # selected_k shape is [batch_size, 1, seq_len, head_dim]
            head_seq_len = selected_k.shape[2] 
            
            k_lens.append(head_seq_len)
            max_seqlen_k = max(max_seqlen_k, head_seq_len)
            klen_sum += head_seq_len
            # ---------------------------------------------------------

            # Flatten to 2D for FlashAttention varlen support
            heads_key_states.append(selected_k.view(-1, head_dim))
            heads_value_states.append(selected_v.view(-1, head_dim))

        _device = past_key_values.key_cache[layer_idx].device

        cluster = model.model.layers[layer_idx].self_attn.kv_cluster
        cluster.init_metadata(num_heads, k_lens, klen_sum, max_seqlen_k, _device)

        # Replace the uncompressed cache with our dynamically pruned, flattened cache
        past_key_values.key_cache[layer_idx] = torch.cat(heads_key_states, dim=0)
        past_key_values.value_cache[layer_idx] = torch.cat(heads_value_states, dim=0)

    import json
    import os
    if hasattr(config, "out_dir") and config.out_dir is not None:
    ...
    print("Cross-layer pruning complete.")
